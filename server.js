...
const PRODUCT_ROWS = [
  ...
  { name: 'ДК/Х5/ИНВ/RE/СНВ/Семейная/Детская', calc: m => num(m.F)+num(m.G)+num(m.H)+num(m.I)+num(m.J)+num(m.K)+num(m.L), group: true },
  { name: 'ДК', calc: m => num(m.F) },
  { name: 'Х5', calc: m => num(m.G) },
  { name: 'ИНВ (DC ИНВ)', calc: m => num(m.H) },
  { name: 'RE', calc: m => num(m.I) },
  { name: 'СНВ (DC нерезидент)', calc: m => num(m.J) },
  { name: 'Семейный', calc: m => num(m.K) },
  { name: 'ДК+Детская', calc: m => num(m.L) },
  { name: 'Активация ДК/RE и др.', calc: m => num(m.M), pctOf: 'ДК/Х5/ИНВ/RE/СНВ/Семейная/Детская' },
  { name: 'Транзакция ДК/RE и др.', calc: m => num(m.N), pctOf: 'ДК/Х5/ИНВ/RE/СНВ/Семейная/Детская' },
  ...
  { name: 'Сэлфи n2b', calc: m => num(m.AC) },
  { name: 'Сэлфи ДК (старая)', calc: m => num(m.AD) },
  ...
  { name: 'Установка МП', calc: m => num(m.AI) },
  { name: 'Установка МП Х5 / по заявке', calc: m => num(m.AR) },
  { name: 'Кросс СИМ', calc: m => num(m.AJ) },
  { name: 'НК (кредит наличными)', calc: m => num(m.AK) },
  ...
  { name: 'УП', calc: m => num(m.CO) },
  { name: 'УП/Отказ банка', calc: m => num(m.AP) },
  { name: 'Альфа-Смарт', calc: m => num(m.AQ) },
  { name: 'ПДС', calc: m => num(m.AS) },
  { name: 'Пенсия', calc: m => num(m.AT) },
  { name: 'Комбо 1₽', calc: m => num(m.AU) }
];
